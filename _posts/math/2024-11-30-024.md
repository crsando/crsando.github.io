...
\begin{aligned}
    x_{m+1} &= \lfloor \alpha x_m \rfloor  \\
        &\geqslant \lfloor \alpha ( \frac{L}{\alpha} + \mu + 1) \rfloor \\
...
引理六：若$L \geqslant x_m > \lceil \frac{L}{\alpha} \rceil$, 则存在一个对应的 $k \geqslant 0$，使得 $x_{m + 2k} = \lceil \frac{L}{\alpha} \rceil$
...
注意到必然有$x_{u+1} > L$（这是因为由于引理3，$x_{u+1} \not = x_u = A$，若$x_{u+1} < L$，则会与$x_u = A$的最大值取法相矛盾），反过来看，有：
...
\begin{aligned}
    ...
    \implies & x_u > \lfloor \frac{L + s}{\alpha} \rfloor \\
    \implies & x_u > \lfloor \frac{L + 1}{\alpha} \rfloor \\
...
若 $x_{u+2k+1} > L$，我们仔细分析一下这个取证的过程，记：
...
$$
    x_{u+2k+1} = L + \lfloor \alpha \mu \rfloor 
$$

由于$x_{u+2k+1} > L$，所以我们知道$\alpha \mu \geqslant 1$，因此

$$
\begin{aligned}
    x_{u+2k+2} &= \lfloor \frac{x_{u+2k+1}}{\alpha} \rfloor \\
        &= \lfloor \frac{L}{\alpha} + 
            \frac{\lfloor \alpha \mu \rfloor}{\alpha}
            \rfloor  \\
        &= \lceil \frac{L}{\alpha} \rceil
             + \lfloor 
                \mu + \frac{\lfloor \alpha \mu \rfloor}{\alpha}
             \rfloor  \\
        &\geqslant \lceil \frac{L}{\alpha} \rceil
             + \lfloor 
                \frac{1}{\alpha} + \frac{\alpha \mu - 1}{\alpha}
             \rfloor  \\
        &\geqslant \lceil \frac{L}{\alpha} \rceil
             + \lfloor \alpha \rfloor  \\
        &> \lceil \frac{L}{\alpha} \rceil
...
则由引理2，我们知道 $x_{u+2k+2} = x_{u+2k} - 1 = \lceil \frac{L}{\alpha} \rceil - 1$，
