...
显然，既然是梯形内部，那么这两个点之间的距离（注意欧式距离必然大于小数距离）不大于该梯形4个边长和两条对角线中的最大值

> 这时候我们大概能猜到，$A_2,A_3$的距离应该不能超过HG的长度

我们现在来说明这个最大值恰好为$HG = d$，则完成证明。为此我们分析一下这个矩形，首先计算一下角度

> 其实结合对称性，很容易猜到

由对称性： $ HM =MG = MK = MI$， 所以$H,G,K,I$四点公圆，圆心为$M$，所以比较线段的弧长，只要找对应的角度就可以了
