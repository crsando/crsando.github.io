...
现在说明必要性：假设这个图不存在孤立的部分，即整个$G(V,E)$是联通的，由于初始状态下的第一步，我们迫使某一行或者某一列发生了旋转或压缩，不妨设为$e_1$，由于至少存在一条通路$e_1 \rightarrow e_2 \rightarrow \cdots e_{2n}$遍历所有的行和列，那么$e_1$带动$e_2$，$e_2$带动$e_3$，依次类推，则所有的行和列都发生了压缩或者旋转。但是，由于我们假设了至少一个格子是保持不变的，构成了矛盾。
