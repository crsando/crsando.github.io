...
勾股定理计算一下$AH = \sqrt{36 - \frac{576}{25}} = \frac{18}{5}$，所以$DH = \frac{43}{5}$，所以
...
[Source](https://www.bilibili.com/video/BV1x15QzyEiE/?spm_id_from=333.1387.upload.video_card.click&vd_source=2c3b1cf87d67c244536d57d4d5b68285)

如图：$\triangle ABC$ 中，$\angle C = 120^\circ$，$\angle A = \angle DEC$，$AC = DE = 6$，$AD = 4$，求 $BC$ 的长。

![](https://crsando.github.io/images/2025-10-15/A-006.png)

***Solution***

这题咋一看会觉得$\triangle ADC$和$\triangle EDC$是$SSA$型全等，但其实有$SSA$，但不是全等，还有一种锐角和钝角的情况。

这题其实就死后一种。碰到这种不要死做$SSA$，解答上是不认的，其实就是往补角上去思考就行了。

我们利用一组等边$DE = AC$一对补角$\triangle BAC + \triangle BED = 180^\circ$，可以把$\triangle BDE$旋转过去得到$\triangle PCA$，那么$P$在$BA$的延长线上，且旋转过去之后$\angle P = \angle B$，所以等腰，所以$BC = PC = BD$（后者是旋转出来的）

其实到了这里就可以了，我们不妨设$BC = BD = t$，那么其实就是解一个三角形$\triangle ABC$，其中大角为$120^\circ$，直接余弦定理即可，如果不用余弦就去做一条高然后勾股，一样的，总之就是

$$
    t^2 + 36 + 6t = (t + 4)^2 = t^2 + 8t + 16 \implies t = 10
$$
